...
def RxMain(params):
    ...
    if not params['use_averaging_before_calculation']:
        ...
        if len(location_list) > (float(record.total_time) / params['time_factor']):
            number_of_groups = int(len(location_list) / (float(record.total_time) / params['time_factor']))
    ...
    utils_obj.ScatterPlot3D(results_dict['X [m]'], results_dict['Y [m]'], results_dict['Z [m]'],
                            ...
                            [(0, params['room_sizes']['x']), (0, params['room_sizes']['y']), (0, params['room_sizes']['z'])])
...
#         results_dict = {}
#     elif algorithm == 3:
#         LUT_obj = algos.RoomMatrix(params['room_size']['x'],
#                                    params['room_size']['y'],
#                                    params['room_size']['z'],
#                                    sp_list,
#                                    res=0.05,
...
#
#         if algorithm == 1:
#             location_list = chan.FilterRoom(location_list,[(0, params['room_size']['x']),
#                                                            (0, params['room_size']['y']),
#                                                            (0, params['room_size']['z'])])
#
#         if len(location_list) > (float(record.total_time) / time_factor):
#             number_of_groups = int(len(location_list)/(float(record.total_time) / time_factor))
#             num_of_elements = int(len(location_list)/number_of_groups)
#
#
#             avg_time = []
...
#     utils_obj.ScatterPlot3D(results_dict['X [m]'],results_dict['Y [m]'], results_dict['Z [m]'],
#                             'Algrithm localization decision - location of microphone', ['X[m]', 'Y[m]', 'Z[m]'],
#                             [(0, params['room_size']['x']), (0, params['room_size']['y']), (0, params['room_size']['z'])])
