...
class Speaker(object):
    ...
    def BuildChirp(self, freqs_dict, Fs):
        ...
        self.unfiltered_signal = {'Fs': Fs, 'low_freq': freqs_dict[str(self.id)][1],
                                  'high_freq': freqs_dict[str(self.id)][0], 'chirp_time': 0.001}

        tmpchirp = signal.chirp(tt, self.unfiltered_signal['high_freq'],
    ...
    def Define_ID(self,my_id, freqs_dict, Fs):
        '''
        ...
        '''
        self.id = my_id
        self.chirp, self.matlab_chirp = self.BuildChirp(freqs_dict, Fs)
...
if __name__ == '__main__':
    ...
    for i in range(len(sp_list)):
        sp_list[i].Define_ID(i+1, speakers_frequencies, Fs)
    ...
    if plotting:
        ...
        #         fftpkt.fftfreq(len(sp.proccessed_signal.filtered_signal), 1.0 / 192000))
